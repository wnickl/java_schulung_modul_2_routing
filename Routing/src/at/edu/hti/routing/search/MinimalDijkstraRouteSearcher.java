 ...
 * needed vogella based objects and re-converts result to internal objects.<br/>
 * <br/>
 * !!only add edges for direction we want to go!!
 * 
 * @author nickl
 * @version $Revision$
 */

public class MinimalDijkstraRouteSearcher implements IRouteSearcher {
  ...
  public IRoute calculate(GraphItem start, GraphItem end, Graph graph) {
    ...
    GraphDirection[] dirs = start.calculateDirection(end);

    for (GraphItem g : graph.getItemsList()) {
      Vertex _g = kurzzeitGedaechtnis.get(g.getXCoordinate() + "-" + g.getYCoordinate());
      /*
       * This is the improvement - only add edges for direction we want to go
       */
      for (GraphItem n : g.getNeighbors(dirs)) {
